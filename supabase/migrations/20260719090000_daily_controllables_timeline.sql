...
BEGIN
  -- Recovery is useful context, but it is a physiological signal rather than
  -- a promise the user kept. Record it without rewarding or punishing identity.
  PERFORM public.upsert_timeline_source_event(
    NEW.user_id,
    'wearable',
...
DECLARE
  ...
  END;
BEGIN
  -- WHOOP sleep has an event-level source with a more accurate timestamp.
...
-- Backfill sources whose no-op updates do not alter user-authored timestamps.
-- Planner and dated-goal history remains authoritative in its source tables;
-- all writes after this migration are synchronized by the triggers above.
...
-- Backfill sources that maintain updated_at without rewriting historical time.
DO $$
BEGIN
...
-- Seed snapshots for the current day. Subsequent source writes and impact
-- changes refresh the exact affected day automatically.
