...
Final review date: 2026-08-01. Statuses: **Covered**, **Partial**, **Deferred**, and **External gate**. “Covered” means implemented and exercised by a test or manual QA. It does not imply pastoral or editorial approval.
...
| Formation before gamification; honest strictness and grace-centered recovery | Covered for circuits; Partial for journeys | Five distinct circuit policies, partial 40-Day saves, Recovery Win, no XP on formation routes | Circuit rule/integration tests and desktop/mobile E2E |
...
| Fully Charged ends an attempt after an incomplete day; new attempts preserve old attempts | Deferred | No authoritative attempt/day-closeout engine exists in this repository | Explicit production launch blocker |
| Seasons change on days 26 and 51; day 75 completes deterministically | Partial | Versioned seed metadata covers days 25/26, 50/51, and 75 | Content validation/seed tests; journey authority deferred |
| Timezone/DST/offline completion cannot falsify authoritative state | Deferred | Circuit writes are server constrained and local QA is labeled, but attempt/calendar authority is absent | Migration contract tests only; live DST/concurrency tests require backend deployment |
...
| Historical covenant/rule/content versions remain unchanged | Partial | Circuit rule version is stored; content version is immutable and retained on existing records | Migration/content tests; authoritative covenant lifecycle remains deferred |
...
| --- | --- | --- |
| Read Along, 40-Day, and 75-Day completion experiences | Covered as test preview; Partial for production | One track-aware route with non-authoritative preview and repository read path; real completion creation awaits attempt engine |
...
| Representative content set | Covered as draft | Requested boundary days, five circuits, five Witness acts, transition, Recovery Win, mission, review, and completion | Seed tests. Samples remain pending human review and intentionally unpublished |
...
| Unit/integration/security/upload/migration/email-render tests | Covered at contract level | 43 files / 273 tests after final changes; includes existing privacy-safe email render tests |
| End-to-end, authorization, accessibility, mobile | Covered for test environment | 10/10 formation E2E across desktop/mobile, Axe, keyboard, privacy and downloads |
...
- **Go:** local and branch-based user testing of the five daily circuits, completion preview, privacy-safe downloads/share preview, and editor tooling.
- **No-go:** production journey completion authority until the attempt/day engine, legacy import/privacy migration, live SQL/RLS verification, and named pastoral/editorial approvals are complete.
