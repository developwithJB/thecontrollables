...
interface ChargeCheckBannerProps {
  signals: GameSignals | null;
}

export function ChargeCheckBanner({ signals }: ChargeCheckBannerProps) {
  if (!signals?.chargeCheck) {
    return null;
  }

  const chargeCheck = signals.chargeCheck;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <h2 className="text-base font-medium text-foreground">{chargeCheck.headline}</h2>
        </div>
      ...
      <p className="text-sm leading-relaxed text-foreground">{chargeCheck.summary}</p>
      ...
      <p className="text-xs leading-relaxed text-muted-foreground">
        Let {atRiskProfile.name} take the lead today. Move through the stretch with care, not force.
      </p>
    </motion.div>
  );
}
