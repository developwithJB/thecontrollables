...
import { GUIDED_JOURNEYS, type GuidedJourney } from "@/lib/guidedJourneys";
...
export function OnboardingJourneySelection({
  ...
  const [selected, setSelected] = useState<string | null>(recommendedJourneyId);
  ...
  const handleContinue = () => {
    const journey = GUIDED_JOURNEYS.find((j) => j.id === selected);
    if (journey) {
      onSelect(journey);
    }
  };
  ...
  const orderedJourneys = [...GUIDED_JOURNEYS].sort((a, b) => {
